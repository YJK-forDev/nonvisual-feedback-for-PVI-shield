...
public class MovementRecognizer_right : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        string[] gestureFiles = Directory.GetFiles(Application.persistentDataPath, "*_right.xml");
        Debug.Log("파일 위치는 : " + Application.persistentDataPath);
    ...
    void Update()
    {
        ...
        if (Vector3.Distance(mainCamera.transform.position, Rcontroller.transform.position) < 0.5f)
        {
            closeToMe = true;
        }
        else
        ...
        {
            StartMovement();
            bg_audio_library.need_punches_r = false;
        }
        //Ending the movement
        else if (isMoving && !isPressed) // 버튼에서 손을 떼면
        {
            EndMovement();
            bg_audio_library.need_success_r = true;
        }
        ...
        {
            bg_audio_library.need_punches_r = true;
        }
    }
    ...
    void EndMovement()
    {
        ...
        if (creationMode)
        {
            ...
            string fileName = Application.persistentDataPath + "/" + newGetureName +"_right" +".xml";
        ...
        else
        {
            ...
            result = PointCloudRecognizer.Classify(newGesture, trainingSet.ToArray()); //입력받은 newGesture를 보고 training set 중 무엇으로 분류되었는지를 results에 저장함 
            Debug.Log("right___"+result.GestureClass + result.Score);
            ...
            switch (result.GestureClass)
            {
                case "Right uppercut":
                    {
                        whichPunch_sound.punch_list_right.Add("uppercut");
                        break;
                    }
                case "Right Jab":
                    {
                        whichPunch_sound.punch_list_right.Add("jab");
                        break;
                    }
                case "Right Hook":
                    {
                        whichPunch_sound.punch_list_right.Add("hook");
                        break;
                    }
            }
        }
