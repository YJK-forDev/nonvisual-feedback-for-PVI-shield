...
public class Oscillator_left : MonoBehaviour
{
    public double frequency = 440.0;
    private double increment;
    private double phase;
    private double sampling_frequency = 90000.0;
    ...
    private double sub_z;
    private double temp = 0;
    private double time;

    Mode_for_myLoca mode;
    AudioSource audiosource;
    MovementRecognizer_left movement_left;
    ...
    void Start()
    {
        ...
        */
        ...
        movement_left = GameObject.Find("Movement Recognizer_left").GetComponent<MovementRecognizer_left>();
    ...
    void FixedUpdate() // 피치, 길이 이런거 조정
    {
        ...
        {
            if (movement_left.punchStart) // 트리거 버튼이 눌리면 audiosource.volume = 1f; (소리가 남)
            {
                ...
                else // 소리가 남
                {
                    ...
                    {
                        if (!movement_left.requestOn) // 버튼이 눌려있지 않다면
                        {
                            audiosource.volume = 0f;
                            gain = gain - sub_z * 0.1; // 값이 커질 수록 볼륨이 커짐
                        }
                    ...
                    {
                        if (movement_left.Movementing)// 펀치 중에 움직이고 있으면
                        {
                            audiosource.volume = 1f;
                            gain = gain - sub_z * 0.1; // 값이 커질 수록 볼륨이 커짐
                        }
                    ...
                    if (mode.trigger_proximity)
                    {
                        ...
                        if (movement_left.closeToMe)// 펀치 중에 움직이고 있으면
                        {
                            audiosource.volume = 1f;
                            gain = gain - sub_z * 0.1; // 값이 커질 수록 볼륨이 커짐
                        }
        ...
        else if (mode.Feedback_mode_Continuous)
        {
            if (movement_left.punchStart)
            {
                ...
                if (mode.trigger_onRequest)
                {
                    if (!movement_left.punchStart) // 펀치 버튼 안눌렀다면
                    {
                        audiosource.volume = 1f;
                        gain = gain - sub_z * 0.1; // 값이 커질 수록 볼륨이 커짐
                    }
    ...
    void OnAudioFilterRead(float[] data, int channels){ // 사인그래프 파형을 그려주는 함수
